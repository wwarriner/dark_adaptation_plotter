...
chk(A, fun, [], [3,1,2]) % Not in help
chk(A, fun, [], [], {{'a',2;'a',10;'a',1},{'.txt';'.txt';'.txt'}}) % not in help
%
P = 'natsortfiles_test';
...
%
C = {'1.9','1.10','1.2'};
chk(C, '\d+\.?\d*',          fun, {'1.2', '1.9', '1.10'}, [3,1,2])
chk(C, '\d+\.?\d*', 'noext', fun, {'1.10', '1.2', '1.9'}, [2,3,1])
%
D = {'B/3.m','A/1.m','B/100.m','A/20m'};
chk(D, '\d+',           fun, {'A/1.m','A/20m','B/3.m','B/100.m'}, [2,4,1,3])
chk(D, '\d+', 'nopath', fun, {'A/1.m','B/3.m','A/20m','B/100.m'}, [2,1,4,3])
%
...
%
F = {'abc2xyz0.txt', 'abc2xyz.txt', 'abc10xyz.txt', 'abc1xyz.txt'};
chk(F, fun, {'abc1xyz.txt','abc2xyz.txt','abc2xyz0.txt','abc10xyz.txt'}, [4,2,1,3])
chk(F, fun, {'abc1xyz.txt','abc2xyz.txt','abc2xyz0.txt','abc10xyz.txt'}, [4,2,1,3], ...
	{{'abc',2,'xyz',0;'abc',2,'xyz',[];'abc',10,'xyz',[];'abc',1,'xyz',[]},...
	{          '.txt';          '.txt';           '.txt';          '.txt'}})
%
...
chk(J, fun,...
    {'1.23V.csv', '1.200V.csv', '010V.csv', '+1.csv', '-1V.csv'}, [1,5,4,3,2])
chk(J, '[-+]?\d+\.?\d*', fun,...
    {'-1V.csv', '+1.csv', '1.200V.csv', '1.23V.csv', '010V.csv'}, [2,3,5,1,4])
%
%% Examples Mfile Help
%
chk({'A1\B', 'A+/B', 'A\B', 'A=/B', 'A\B0'}, fun,... X
    {'A\B', 'A\B0', 'A1\B', 'A+/B', 'A=/B'}, [3,5,1,2,4])
